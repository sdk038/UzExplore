  ...
  return (
    <div className="min-h-screen bg-stone-50 text-slate-900">
      ...
      <main id="destination-root" className="bg-stone-50">
        ...
        <section className="destination-section practical-grid"><article><span>🚆</span><h3>Getting there</h3><p>Travel from Tashkent by train, intercity bus or domestic flight. A transfer is best for nature routes.</p></article><article><span>🍽</span><h3>What to try</h3><p>Regional plov, samsa, fresh bread, seasonal fruit and dishes from local teahouses.</p></article><article><span>🧭</span><h3>Before you go</h3><p>Dress respectfully at religious sites and carry water and sun protection in summer.</p></article></section>
        <section className="destination-section destination-section--tint" id="hotels"><div className="destination-heading"><span className="section-kicker">Accommodation</span><h2>Where to stay</h2><p>Three useful starting points for planning your visit.</p></div><div className="service-grid">{hotelPhotos.map((photo,index)=><article className="service-card" key={photo}><div className="service-card__photo" style={{backgroundImage:`url('${photo}')`}} /><span>★ {(4.9-index*.1).toFixed(1)}</span><h3>{index===0?`${city.name} Silk Road Residence`:index===1?'Old City Boutique':'UzExplore City Hotel'}</h3><p>Convenient location · Wi-Fi · Breakfast</p><strong>Price depends on travel dates</strong><a className="btn btn-primary" href={`https://www.google.com/maps/search/hotels+${encodeURIComponent(city.name)}`} target="_blank" rel="noopener noreferrer">View hotels</a></article>)}</div></section>
        <section className="destination-section" id="mobility"><div className="mobility-panel"><div><span className="section-kicker">Getting around</span><h2>Local transport</h2><p>Taxis and local transfers are available in the city; choose a driver or guided tour for remote sites.</p><ul><li>City taxis and transfers</li><li>Walking routes in the center</li><li>Driver service for day trips</li></ul></div><a className="btn btn-light" href={`https://www.google.com/maps?q=${encodeURIComponent(city.center)}`} target="_blank" rel="noopener noreferrer">Open map</a></div></section>
      ...
      <footer className="footer"><div className="footer-inner"><p>UzExplore — your route across Uzbekistan</p><p className="copyright">© 2026 UzExplore</p></div></footer>
    </div>
  );
}
